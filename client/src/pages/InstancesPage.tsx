...
export default function InstancesPage() {
  ...
  const handleExport = async (instance: Instance) => {
    ...
    try {
      ...
      // JPEG: the browser produces an already-compressed image, so jsPDF embeds
      // it directly instead of deflate-compressing a huge PNG (which froze the
      // main thread ~17s). Same resolution (multiplier 3 ≈ 285 dpi).
      const dataURL = tempCanvas.toDataURL({ format: 'jpeg', quality: 0.95, multiplier: 3 });
...
function InstanceCard({
  ...
  return (
    <div className="card group">
      <div className="aspect-[3/4] bg-white p-2 rounded-t-xl overflow-hidden cursor-pointer border-b border-gray-200" onClick={onEdit}>
        {instance.thumbnail ? (
          <img
